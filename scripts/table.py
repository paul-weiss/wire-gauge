...
Keeps only the newest record per (machine, backend, scenario, rate, size) —
...
def main() -> None:
    ...
    hdr = f"{'machine':>8} {'backend':>9} {'rate/s':>7} {'size':>5} {'recv':>9} {'drop':>6} " \
    ...
    for r in rows:
        ...
        print(
            f"{r['machine']['hostname']:>8} {r['backend']:>9} {r['config']['rate']:>7} "
